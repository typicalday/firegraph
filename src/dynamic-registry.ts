 ...
 *
 * Memoized at module scope: `BOOTSTRAP_ENTRIES` are frozen module-level
 * constants and `createRegistry` is pure over them, so the resulting
 * registry — including its compiled cfworker `Validator`s — can be
 * reused across every `GraphClientImpl` constructor. This matters on
 * Cloudflare Workers, where the dynamic client constructor runs on
 * every request that touches the meta-registry path; without
 * memoization we'd re-walk + re-dereference these schemas per request.
 */
