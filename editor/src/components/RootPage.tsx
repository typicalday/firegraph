...
import { encodeFsPath, getTypeBadgeColor } from '../utils';
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {!hasAnything && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center">
          <p className="text-sm text-slate-400">
            No graph collection or collections configured.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        {graphCollection && (
          ...
          >
            ...
            <p className="text-xs text-slate-500">
              Graph &middot; {schema.nodeTypes.length} node type{schema.nodeTypes.length !== 1 ? 's' : ''}, {schema.edgeTypes.length} edge type{schema.edgeTypes.length !== 1 ? 's' : ''}
            </p>
          </Link>
        )}
        ...
        {topLevelCollections.map((col) => (
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <svg className="w-4 h-4 text-amber-400/70 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
