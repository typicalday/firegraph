...
export default function CollectionDocDetail({
  collectionDef,
  docId,
  params,
  readonly,
  ...
  if (error) {
    return <div className="px-6 py-4 text-sm text-red-400">{error.message}</div>;
  }
            ...
            <div className="flex items-center gap-2">
              ...
              {!confirmDelete ? (
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    onClick={() =>
                      deleteMutation.mutate({ collectionName: collectionDef.name, params, docId })
                    }
                        ...
                        {data.data[f.name] !== undefined ? (
                          typeof data.data[f.name] === 'object' ? (
                            <code className="text-[10px] font-mono text-slate-400">
                              {JSON.stringify(data.data[f.name])}
                            </code>
                          ) : (
