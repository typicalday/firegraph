...
describe('Per-backend capability unions', () => {
  it('FirestoreStandardCapability is the conservative core surface plus raw.firestore', () => {
    type Expected =
      | 'core.read'
      | 'core.write'
      | 'core.transactions'
    ...
    // Phases 4-10 must NOT appear in the Standard union. Using
    // `not.toMatchTypeOf` (per-cap) catches a regression that adds, e.g.,
    // 'query.aggregate' to STANDARD_CAPS without the matching method.
    expectTypeOf<FirestoreStandardCapability>().not.toMatchTypeOf<'query.aggregate'>();
  ...
  it('FirestoreEnterpriseCapability matches Standard until Phases 4-10 add extensions', () => {
    type Expected =
      | 'core.read'
      | 'core.write'
      | 'core.transactions'
    ...
    // Same negative parity as Standard — extensions arrive in Phases 4-10.
    expectTypeOf<FirestoreEnterpriseCapability>().not.toMatchTypeOf<'query.aggregate'>();
  ...
  it('SqliteCapability declares interactive transactions statically (runtime cap-set varies by driver)', () => {
    type Expected =
      | 'core.read'
      | 'core.write'
      | 'core.transactions'
  ...
  it('CloudflareCapability omits transactions and raw.sql by design', () => {
    type Expected = 'core.read' | 'core.write' | 'core.batch' | 'core.subgraph';
...
describe('GraphClient<C> assignability tracks the capability union', () => {
  ...
  it('a narrower client is structurally assignable to a wider one while extension interfaces are empty', () => {
    // Both sides share `CoreGraphClient`, so each can flow into a
    // variable typed as the other's union — the conditional extensions
    // are all empty interfaces in Phase 3. When Phase 4+ adds methods
    // to an extension, this assertion will start to fail (intentionally:
    // a client with `query.aggregate` is no longer assignable to one
    // without it). At that point the test should be split into one
    // assertion per direction with the wider→narrower flow rejected.
  ...
  it('GraphClient<core-only-cap> exposes only CoreGraphClient keys (no extension leak)', () => {
    ...
    // and once Phase 4 adds a real method to that extension, this test
    // will catch it.
  ...
  it('Cloudflare client (no extension caps declared) is locked to CoreGraphClient keys', () => {
    // CloudflareCapability deliberately omits every extension cap. No
    // matter what methods Phases 4-10 add to `AggregateExtension`,
    // `DmlExtension`, etc., the key set of `GraphClient<CloudflareCapability>`
    // must remain `keyof CoreGraphClient`. This is the test that survives
    // the empty-extension era and guards the Cloudflare contract going
    // forward.
    expectTypeOf<keyof DOClient>().toEqualTypeOf<keyof CoreGraphClient>();
    expect(true).toBe(true);
  });
});
