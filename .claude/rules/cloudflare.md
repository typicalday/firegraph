...
| `src/cloudflare/sql.ts`                    | SQLite DDL/DML for the DO's `state.storage.sql` interface                |
| `src/cloudflare/schema.ts`                 | Schema bootstrap inside the DO                                           |
| `src/cloudflare/index.ts`                  | Public barrel — exports `FiregraphDO`, `DORPCBackend`, `createDOClient`  |
